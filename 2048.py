from Twinkter import Frame,Label,CENTER
...
class Game2048(Frame):
    ...
    def init_grid(self):
        ...
        for i in range(c.GRID_LEN):
            grid_row=[]
            for j in rnge(c.GRID_LEN):
                cell=Frame(background,bg=c.BACKGROUND_COLOR_CELL_EMPTY,
                width=c.SIZE/C.GRID_LEN,height=c.SIZE/c.GRID_LEN)

                cell.grid(row=i,coloumn=j,padx=c.GRID_PADDING,pady=c.GRID_PADDING)
...
def init_matrix(self):
    self.init_matrix=LogicsFinal.start_game()
...
def key_down(self,event):
    ...
    if key in self.commands:
        ...
        if changed:
            ...
            self.update_grid_cells()
            changed=False
            if LogicsFinal.get_current_game(self.matrix)=="WON":
                self.grid_cells[1][1].configure(text="YOU",bg=c.BACKGROUND_COLOR_CELL_EMPTY)
                self.grid_cells[1][2].configure(
                    text="WIN!",bg=c.BACKGROUND_COLOR_CELL_EMPTY
                )
                if LogicsFinal.get_current_game(self.matrix)=="LOST":
